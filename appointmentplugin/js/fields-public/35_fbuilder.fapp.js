...
$.extend( 
	...
	$.fbuilder.controls[ 'ffields' ].prototype,
	{
		...
		getCompatSlots:function(d)
		{
			...
			show:function()
			{
				return '<div class="fields '+this.csslayout+'" id="field'+this.form_identifier+'-'+this.index+'"><label for="'+this.name+'">'+this.title+''+((this.required)?"<span class='r'>*</span>":"")+'</label><div class="dfield"><input class="field avoid_overlapping_before '+((this.required)?" required":"")+'" id="'+this.name+'" name="'+this.name+'" type="hidden" value="" summary="usedSlots"/><input id="'+this.name+'_services" name="'+this.name+'_services" type="hidden" value="0"/><input id="'+this.name+'_capacity" name="'+this.name+'_capacity" type="hidden" value="0"/><input class="" id="tcost'+this.name+'" name="tcost'+this.name+'" type="hidden" value=""/><div class="step1"><h2>STEP 1</h2><p>Choose a date</p><div class="fieldCalendarService'+this.name+'"></div><div class="fieldCalendar'+this.name+'"></div><div class="available_symbol"><div class="not_available">Not Available</div><div class="available">Available</div><div class="selected">Selected</div></div></div><div class="step2"><h2>STEP 2</h2><p>Choose available time</p><div class="note_hours">Note: One hour is booked.</div><div class="time-wrapper"><div class="select_time"><button type="button" class="time_s" onclick="select_time_from_slot();">Select Time</button><script>  function select_time_from_slot(){ $(".slotsCalendar'+this.name+'").toggle(); } </script></div><div class="slotsCalendar'+this.name+'" style="display:none"></div></div><div class="usedSlots'+this.name+'"></div><span class="uh">'+this.userhelp+'</span><?php if(!is_user_logged_in()){ ?><div class="book_session_text">In order to book a session, you need to be registered or signed in.</div>  <ul><li><a class="btn btn-outline <?php if(is_user_logged_in()){ echo "disabled"; } ?>" href="<?php echo site_url(); ?>/login/" >Log In</a></li><li><a class="btn btn-orange <?php if(is_user_logged_in()){ echo "disabled"; } ?>" href="<?php echo site_url(); ?>/registrations/">Register</a></li></ul><?php }else{ ?><div class="pbSubmit"><span>Submit</span></div> </div><?php } ?><div class="clearer"></div></div>';
			},
		    ...
		    after_show:function()
		    {
		    	...
		    	function renderCalendarCallback(d) {
		    		...
		    		var currentdate_get = $.datepicker.formatDate('yy-mm-dd', new Date());
                   ...
                   if ($.inArray(day, me.invalidDatesByService["s"+me.service_selected+"q"+me.quantity_selected]) > -1 && me.notShowBookedDate)
                   	if(currentdate_get >= day){
                   		c.push("nonworking","ui-datepicker-unselectable","ui-state-disabled","notavailslot");
                   	}else{
                   		c.push("nonworking","ui-datepicker-unselectable","ui-state-disabled","notavailslot","blockdate");
                   	}
                   ...
                   function onChangeDateOrService(d)
                   {
                   	...
                   	{		
                   		...
                   		for (var i=0;i<me.allUsedSlots.length;i++)
                   		{
                   			...
                   			$('.time_s').text(me.formatString(me.allUsedSlots[i],false,me.tz));
                   			$('.time_s').trigger('click');	  			    
                   ...
                   d.html('<select class="ahbfield_service" style="display:none">'+str+'</select><div class="ahbfield_quantity_div" '+((!me.showQuantity)?"style='display:none'":"")+'><label class="ahbfield_quantity_label">'+((typeof cp_hourbk_quantity_label !== 'undefined')?cp_hourbk_quantity_label:'Quantity')+'</label><br /><select class="ahbfield_quantity">'+str2+'</select></div>');
